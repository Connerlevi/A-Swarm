...
# Robust version with proper waits and comprehensive fixes
...
Write-Host "=== Docker Desktop Kubernetes Recovery Script (Robust Edition) ===" -ForegroundColor Cyan
...
if ($dockerExe) {
    ...
    if (-not (Wait-DockerDesktop -TimeoutSec 240)) {
        Write-Host "[ERROR] Docker engine failed to start" -ForegroundColor Red
...
if (Wait-Kubernetes -TimeoutSec 240) {
    ...
    Write-Host "[ERROR] Kubernetes did not become ready in time" -ForegroundColor Red
...
if ($logs -match "PodOK") {
    Write-Host "[OK] Pod creation test PASSED" -ForegroundColor Green
} else {
    Write-Host "[ERROR] Pod creation test FAILED" -ForegroundColor Red
