...
def main(ns, selector="app=anomaly", repeat=1):
    ...
    for run_num in range(repeat):
        ...
        # Pass run_id to pheromone via ConfigMap
        cm_manifest = {
            "apiVersion": "v1",
            "kind": "ConfigMap",
            "metadata": {"name": "run-config", "namespace": ns},
            "data": {"run_id": run_id}
        }
        subprocess.run(['kubectl','apply','-f','-'], input=json.dumps(cm_manifest).encode(), check=True)
        
        # Wait for anomaly to start
        time.sleep(2)
        ...
        # wait until pheromone elevates (using run-specific configmap)
        ...
        for _ in range(60):
            try:
                out = subprocess.check_output(['kubectl','-n',ns,'get',f'cm/aswarm-elevated-{run_id}','-o','json']).decode()
