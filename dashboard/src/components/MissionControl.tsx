...
// ---- Data Context for isolating streaming data ----
interface DataContextType {
  events: Array<{ts: string, level: 'info' | 'warning' | 'error', message: string}>
  episodes: Record<string, Episode>
  fleet: FleetStatus
  rules: RuleItem[]
  epHistory: Record<string, Array<{ts: string, status: Episode["status"], note?: string}>>
  killEngaged: boolean
  approval1: boolean
  approval2: boolean
  ttl: number
  ...
  reloadRules: (rules: RuleItem[]) => void
  setKillEngaged: (engaged: boolean) => void
  setApproval1: (approval: boolean) => void
  setApproval2: (approval: boolean) => void
}
...
function useDemoFeed(enabled: boolean, push: (msg: any) => void) {
  ...
  useEffect(() => {
    console.log('[DEBUG] useDemoFeed - enabled:', enabled);
    if (!enabled) return;
    console.log('[DEBUG] Starting demo data feed...');
    const tid = setInterval(() => {
      const now = new Date().toISOString();
      console.log('[DEBUG] Generating demo data at', now);
      // Random event
      const levels = ["info", "warning", "error"] as const;
      const level = levels[Math.floor(Math.random() * levels.length)];
      pushRef.current({ type: "event", level, message: `${level.toUpperCase()}: heartbeat at ${now}` });
      // Random episode update
      if (Math.random() < 0.4) {
        const status = ["detected", "running", "contained", "resolved"][Math.floor(Math.random() * 4)] as Episode["status"];
        pushRef.current({ type: "episode", id: `ep-${Math.floor(Math.random() * 5) + 1}`, attacklet_name: "data-exfiltration-v2", status, ttd_seconds: Math.random() * 0.3, score: Math.floor(Math.random() * 100) });
      }
      // Random fleet
      if (Math.random() < 0.5) {
        ...
        pushRef.current({ type: "fleet_status", total: nodes.length, healthy: nodes.filter(n => n.status === "healthy").length, avg_cpu: +(nodes.reduce((a, n) => a + n.cpu, 0) / nodes.length).toFixed(1), avg_memory: Math.floor(nodes.reduce((a, n) => a + n.memory, 0) / nodes.length), nodes });
      }
    }, 1500);
    return () => clearInterval(tid);
  }, [enabled]); // Removed push dependency
}
...
function useASwarmWS(onMsg: (data: any) => void) {
  ...
  // TEMPORARY: Disable WebSocket completely to use demo mode
  useEffect(() => {
    console.log('[DEBUG] WebSocket disabled - using demo mode');
    setConnected(false);
  }, []);
...
// Original WebSocket implementation (disabled for now)
function useASwarmWS_DISABLED(onMsg: (data: any) => void) {
  const [connected, setConnected] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    function connect() {
      ...
      if (attemptsRef.current >= 3) {
        console.log('[DEBUG] Max WebSocket attempts reached, staying in demo mode');
        setConnected(false);
        return;
      }

      try {
        console.log('[DEBUG] WebSocket connection attempt', attemptsRef.current + 1);
        ...
        ws.onclose = () => {
          ...
          if (alive && attemptsRef.current < 3) {
            console.log('[DEBUG] Will retry WebSocket in 1.5s');
            retryTimeout = setTimeout(connect, 1500);
          } else {
            console.log('[DEBUG] Giving up on WebSocket, using demo mode');
          }
        };

        ws.onerror = (err) => {
          console.log('[DEBUG] WebSocket error - will close and retry');
        };
        ...
        console.log('[DEBUG] WebSocket creation failed:', err);
        setConnected(false);
        attemptsRef.current++;
...
function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  // Stable handleMsg function using useRef to prevent re-renders
  const toastRef = useRef(toast);
  toastRef.current = toast;

  const handleMsg = useRef((data: any) => {
    switch (data.type) {
      case "event": {
      ...
      case "rules_update": {
        setRules(data.rules ?? []);
        toastRef.current({ title: "Rules reloaded", description: new Date().toLocaleTimeString() });
        break;
      }
      case "kill_switch": {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (left === 0) {
        ...
        setApproval1(false);
        setApproval2(false);
        toastRef.current({ title: "Kill switch auto-reverted", description: "TTL expired, operations restored." });
        clearInterval(id);
      }
    }, 1000);
  ...
  // Stable handlers
  const updateRules = useCallback((newRules: RuleItem[]) => {
    setRules(newRules);
  }, []);

  const reloadRules = useCallback((rules: RuleItem[]) => {
  ...
  const contextValue: DataContextType = useMemo(() => ({
    ...
    setApproval1,
    setApproval2,
  }), [events, episodes, fleet, rules, epHistory, killEngaged, approval1, approval2, ttl, connected, updateRules, reloadRules]);
...
// ---- Hoisted UI Components (module scope to prevent re-mounting) ---- FIXED
const MetricCard = React.memo(function MetricCard({ icon: Icon, label, value, sub }: { icon: any; label: string; value: string; sub?: string }) {
  return (
...
const HeaderBar = React.memo(function HeaderBar({ connected }: { connected: boolean }) {
  return (
...
const KillSwitchCard = React.memo(function KillSwitchCard() {
  const { killEngaged, approval1, approval2, ttl, setKillEngaged, setApproval1, setApproval2 } = useDataContext();
  const { toast } = useToast();
...
const EpisodesPanel = React.memo(function EpisodesPanel() {
  const { episodes, epHistory } = useDataContext();
  ...
  const list = useMemo(() => Object.values(episodes).sort((a, b) => (b.started_at || "").localeCompare(a.started_at || "")), [episodes]);
...
const FleetPanel = React.memo(function FleetPanel() {
  const { fleet } = useDataContext();
...
const RulesPanel = React.memo(function RulesPanel() {
  const { rules, updateRules, reloadRules } = useDataContext();
  const [searchRules, setSearchRules] = useState("");

  useEffect(() => {
    console.log('[DEBUG] RulesPanel mounted');
    return () => console.log('[DEBUG] RulesPanel unmounted!!!');
  }, []);

  console.log('[DEBUG] RulesPanel render, searchRules:', searchRules);

  const filtered = useMemo(() =>
    rules.filter(r =>
      r.name.toLowerCase().includes(searchRules.toLowerCase()) ||
      (r.technique ?? "").toLowerCase().includes(searchRules.toLowerCase())
    ),
    [rules, searchRules]
  );

  const handleRuleToggle = useCallback((rule: RuleItem, enabled: boolean) => {
    const updatedRules = rules.map(r => r === rule ? { ...r, enabled } : r);
    ...
    <Card className="border-slate-800 bg-slate-950">
      <CardHeader className="flex gap-2">
        ...
        <div className="flex items-end gap-2">
          <Button variant="outline" className="border-slate-700" onClick={handleReload}><RefreshCw className="h-4 w-4 mr-2" />Reload</Button>
        </div>
      </CardHeader>
      <CardContent>
        <div className="mb-3 flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder="Search by name or ATT&CK technique"
...
const EventStream = React.memo(function EventStream() {
  const { events } = useDataContext();
  const [filter, setFilter] = useState<'all' | 'info' | 'warning' | 'error'>("all");

  const list = useMemo(() =>
    events.filter(e => filter === 'all' ? true : e.level === filter),
    [events, filter]
  );
...
// ---- Main UI Component (no longer subscribes to volatile context) ----
function MissionControlUI(): JSX.Element {
  const { connected } = useDataContext();
