...
export default function MissionControl(): JSX.Element {
  ...
  // Static demo data - this never changes so no re-render issues
  const [episodes] = useState<Record<string, Episode>>({
    "ep-1": { id: "ep-1", attacklet_name: "privilege-escalation-v3", status: "contained", ttd_seconds: 0.08, score: 95, technique: "T1068", started_at: "2025-09-15T02:30:00.000Z" },
    "ep-2": { id: "ep-2", attacklet_name: "data-exfiltration-v2", status: "detected", ttd_seconds: 0.06, score: 88, technique: "T1041", started_at: "2025-09-15T02:29:30.000Z" },
  });

  const [fleet] = useState<FleetStatus>({
    total: 12,
    healthy: 11,
    avg_cpu: 1.2,
    avg_memory: 18,
    nodes: Array.from({ length: 12 }).map((_, i) => ({
      ...
      cpu: +(0.8 + Math.random() * 1.5).toFixed(1),
      memory: Math.floor(16 + Math.random() * 8),
    }))
  });
  ...
  const [events] = useState([
    { ts: "02:30:15", level: "info" as const, message: "INFO: System health check completed" },
    { ts: "02:30:12", level: "warning" as const, message: "WARNING: High CPU usage on node-4" },
    { ts: "02:30:08", level: "error" as const, message: "ERROR: Privilege escalation detected on node-1" },
    { ts: "02:30:05", level: "info" as const, message: "INFO: Detection rules v2.0.1 loaded successfully" },
  ]);
  ...
  const connected = false; // Demo mode
        ...
        <Dialog open={timelineOpen} onOpenChange={setTimelineOpen}>
          <DialogContent className="max-w-lg bg-slate-950 border-slate-800">
            ...
            <div className="max-h-[50vh] overflow-auto pr-2">
              <div className="text-xs text-slate-500">Static demo - no timeline data</div>
            </div>
          </DialogContent>
        </Dialog>
      </Card>
      ...
      <Card className="border-slate-800 bg-slate-950">
        <CardHeader className="flex gap-2">
          ...
          <div className="flex items-end gap-2">
            <Button variant="outline" className="border-slate-700" onClick={() => toast({ title: "Rules reloaded", description: new Date().toLocaleTimeString() })}><RefreshCw className="h-4 w-4 mr-2" />Reload</Button>
          </div>
        </CardHeader>
        <CardContent>
                ...
                <div key={rule.id ?? rule.name} className={`flex items-center justify-between rounded-md border p-2 ${sevColor[rule.severity]}`}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(enabled) => setRules(rs => rs.map(r => r === rule ? { ...r, enabled } : r))}
                    />
                  </div>
                </div>
              ))}
        ...
        <CardContent>
          <ScrollArea className="h-40 pr-2">
            <div className="space-y-1 text-xs">
              ...
              {list.length === 0 && <div className="text-slate-500">No events for this filter.</div>}
            </div>
          </ScrollArea>
        </CardContent>
      </Card>
